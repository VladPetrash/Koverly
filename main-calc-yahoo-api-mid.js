...
window.addEventListener("alpine:init", () => {
   ...
   Alpine.data("exchanger", () => ({
       ...
       async refreshData(callback) {
           ...
           let minRate = result.results[0].low;
           let maxRate = result.results[0].high;
           this.data.rate = (minRate + maxRate) / 2;
           let minInverseRate = inverseResult.results[0].low;
           let maxInverseRate = inverseResult.results[0].high;
           this.data.inverseRate = (minInverseRate + maxInverseRate) / 2;
           console.log("minRate: " + minRate + " maxRate: " + maxRate);
           console.log("minInverseRate: " + minInverseRate + " maxInverseRate: " + maxInverseRate);
