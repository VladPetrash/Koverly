    ...
    Alpine.data("calc", () => ({
        ...
        converter() {
            let setInit = async () => {
                ...
                if (this.data.option === "buy") {
                    // if value from selected on previous page -> set it on current page
                    if (
                        localStorage.hasOwnProperty("inputBuy") &&
                        Number(localStorage.getItem("inputBuy")) != 0
                    ) {
                        this.data.inputBuy = localStorage.getItem("inputBuy");
                        console.log(this.data.inputBuy);
                        this.buyValue();
                        localStorage.removeItem("inputBuy");
                    } else {
                        this.buyValue();
                    }
                }
                if (this.data.option === "sell") {
                    // if value from selected on previous page -> set it on current page
                    if (
                        localStorage.hasOwnProperty("inputSell") &&
                        Number(localStorage.getItem("inputSell")) != 0
                    ) {
                        this.data.inputBuy = localStorage.getItem("inputSell");
                        console.log(this.data.inputBuy);
                        this.sellValue();
                        localStorage.removeItem("inputSell");
                    } else {
                        this.sellValue();
                    }
                }
        ...
        convertBuy() {
            localStorage.setItem("valueFrom", this.data.inputBuy);
            this.buyValue();
        },

        buyValue() {
            this.data.inputResult4 = parseFloat(`${this.data.inputBuy}`.replace(/,/g, "")) * this.data.rate;
            this.data.inputResult8 = parseFloat(`${this.data.inputBuy}`.replace(/,/g, "")) * this.data.rate;
            this.data.inputResult12 = parseFloat(`${this.data.inputBuy}`.replace(/,/g, "")) * this.data.rate;
            this.data.inputResult4 = (this.data.inputResult4 + (1.5 / 100) * this.data.inputResult4) / 4;
            this.data.inputResult8 = (this.data.inputResult8 + (3 / 100) * this.data.inputResult8) / 8;
            this.data.inputResult12 = (this.data.inputResult12 + (4.5 / 100) * this.data.inputResult12) / 12;
            this.data.inputResult4 = this.data.inputResult4.toLocaleString("en-US", {
            		minimumFractionDigits: 2,
                maximumFractionDigits: 2,
            });
            this.data.inputResult8 = this.data.inputResult8.toLocaleString("en-US", {
            		minimumFractionDigits: 2,
                maximumFractionDigits: 2,
            });
            this.data.inputResult12 = this.data.inputResult12.toLocaleString("en-US", {
            		minimumFractionDigits: 2,
                maximumFractionDigits: 2,
            });
            console.log(this.data.inputBuy);
            console.log(this.data.inputResult4);
            this.data.preloaderShow = false;
        },
    }));
});
