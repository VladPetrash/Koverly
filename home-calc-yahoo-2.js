...
window.addEventListener("alpine:init", () => {
    ...
    Alpine.data("exchanger", () => ({
        ...
        async refreshData(callback) {
            ...
            try {
                const response = await fetch(
                    `https://yahoofinance-stocks1.p.rapidapi.com/stock-metadata?Symbol=${this.data.buyCurrency.currencyName}%3DX`,
                    {
                        method: "GET",
                        headers: {
                            "X-RapidAPI-Key":
            ...
            this.data.rate = result.result.ask;
        ...
        sellValue() {
            ...
            this.data.totalFrom =
                this.data.valueFromNumber - this.data.rateLock;
            this.data.valueToNumber = this.data.totalFrom * this.data.rate;
            this.data.valueTo = this.data.valueToNumber.toLocaleString(
                "en-US",
                { maximumFractionDigits: 2 }
            );
            this.data.rateLock = this.data.rateLock.toLocaleString("en-US", {
                maximumFractionDigits: 2,
            });
            this.data.totalFrom = this.data.totalFrom.toLocaleString("en-US", {
                maximumFractionDigits: 2,
            });
            console.log("sell");
        },
        ...
        buyValue() {
            ...
            this.data.valueFromNumber =
                this.data.valueToNumber / this.data.rate / 0.999;
            this.data.rateLock = this.data.valueFromNumber * 0.001;
            this.data.totalFrom =
                this.data.valueFromNumber - this.data.rateLock;
            this.data.valueFrom = this.data.valueFromNumber.toLocaleString(
                "en-US",
                { maximumFractionDigits: 2 }
            );
            this.data.rateLock = this.data.rateLock.toLocaleString("en-US", {
            ...
            console.log(
                "buy",
                "valueFromNumber = " + this.data.valueFromNumber,
