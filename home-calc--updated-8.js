...
window.addEventListener("alpine:init", () => {
    ...
    Alpine.data("exchanger", () => ({
        ...
        async refreshData(callback) {
            ...
            this.data.rate = 1 / result.rate;
            this.data.inverseRate = 1 * result.rate;
            this.data.rateLimited = this.data.rate.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 5});
            this.data.inverseRateLimited = this.data.inverseRate.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 5});
            if (callback) {
                callback.apply(this);
            }
        },
